...
namespace zip_utils::detail::utils {
    ...
    using types_to_rvalues_mask =
        bool_mask<std::is_rvalue_reference_v<Types> ...>;

    template <typename ... Types>
    consteval bool all_are_lvalues_or_nothrow_movable() {
        return ((std::is_lvalue_reference_v<Types> ||
            std::is_nothrow_move_constructible_v<std::decay_t<Types>>) && ...);
    }
